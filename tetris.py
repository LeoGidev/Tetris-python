...
def valid_space(piece, grid):
    for x, y in convert_shape_format(piece):
        if y > -1 or x < 0 or x >= PLAY_WIDTH or grid[y][x] != BLACK:
            return False
    return True
    ...
    while True:
        ...
        for y in range(len(grid)):
            for x in range(len(grid[y])):
                pygame.draw.rect(screen, grid[y][x], (TOP_LEFT_X + x * BLOCK_SIZE, TOP_LEFT_Y + y * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE), 0)
